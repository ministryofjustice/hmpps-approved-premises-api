...
class Cas1ChangeRequestServiceTest {
  ...
  @Test
  fun `throws general validation error when change request type is not appeal or planned transfer`() {
    val cas1SpaceBooking = Cas1SpaceBookingEntityFactory().produce()
    ...
    val cas1NewChangeRequest = Cas1NewChangeRequestFactory()
      .withType(Cas1ChangeRequestType.EXTENSION)
      .withReasonId(cas1ChangeRequestReason.id)
      .withSpaceBookingId(cas1SpaceBooking.id)
      .produce()
    ...
    assertThatCasResult(result).isGeneralValidationError("Change request type is not PLANNED_TRANSFER or APPEAL")
  }
