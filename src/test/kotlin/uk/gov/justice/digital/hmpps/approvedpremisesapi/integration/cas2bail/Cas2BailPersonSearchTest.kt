...
class Cas2BailPersonSearchTest : IntegrationTestBase() {
  @Nested
  inner class Cas2BailPeopleSearchGet {

    @Nested
    inner class WhenThereIsAnError {
      @Test
      fun `Searching cas2bail by NOMIS ID without a JWT returns 401`() {
        webTestClient.get()
          .uri("/cas2bail/people/search?nomsNumber=nomsNumber").exchange()
          .expectStatus()
          .isUnauthorized
      }

      @Test
      fun `Searching cas2bail for a NOMIS ID with a non-Delius or NOMIS JWT returns 403`() {
        ...
        webTestClient.get()
          .uri("/cas2bail/people/search?nomsNumber=nomsNumber")
      ...
      @Test
      fun `Searching cas2bail for a NOMIS ID without ROLE_POM returns 403`() {
      ...
      @Test
      fun `Searching cas2bail for a NOMIS ID returns Unauthorised error when it is unauthorized by the API`() {
        givenACas2PomUser { userEntity, jwt ->
          probationOffenderSearchAPIMockForbiddenOffenderSearchCall()

          webTestClient.get()
            .uri("/cas2bail/people/search?nomsNumber=NOMS321")
            .header("Authorization", "Bearer $jwt")
            .exchange()
            .expectStatus()
            .isForbidden
      ...
      @Test
      fun `Searching cas2bail for a NOMIS ID returns unauthorised error when offender is in a different prison`() {
        givenACas2PomUser { userEntity, jwt ->
          ...
          webTestClient.get()
            .uri("/cas2bail/people/search?nomsNumber=NOMS456")
            .header("Authorization", "Bearer $jwt")
            .exchange()
            .expectStatus()
            .isForbidden
      ...
      @Test
      fun `Searching cas2bail for a NOMIS ID returns 404 error when it is not found`() {
        givenACas2PomUser { userEntity, jwt ->
      ...
      @Test
      fun `Searching cas2bail for a NOMIS ID returns server error when there is a server error`() {
        givenACas2PomUser { userEntity, jwt ->
    ...
    inner class WhenSuccessful {
      @Test
      fun `Searching cas2bail for a NOMIS ID returns OK with correct body`() {
