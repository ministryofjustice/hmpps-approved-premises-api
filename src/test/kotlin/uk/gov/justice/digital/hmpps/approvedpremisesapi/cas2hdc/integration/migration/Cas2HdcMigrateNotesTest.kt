...
class Cas2HdcMigrateNotesTest : MigrationJobTestBase() {
  ...
  fun `Should add assessments for CAS2 notes that don't have one and returns 202 response`() {
    givenACas2PomUser { userEntity, _ ->
      givenACas2Assessor { assessor, _ ->
        val submittedApplication1 = createApplicationEntity(userEntity, OffsetDateTime.now().minusDays(1))
        ...
        val submittedApplication2 = createApplicationEntity(userEntity, OffsetDateTime.now().minusDays(1))
