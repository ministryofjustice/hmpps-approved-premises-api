...
import uk.gov.justice.digital.hmpps.approvedpremisesapi.service.cas1.domainevent.PlacementAppealAcceptedTimelineFactory
import java.time.LocalDate
import java.util.UUID

@ExtendWith(MockKExtension::class)
class PlacementAppealAcceptedTimelineFactoryTest {
  @MockK
  lateinit var domainEventService: Cas1DomainEventService

  @InjectMockKs
  lateinit var service: PlacementAppealAcceptedTimelineFactory
  ...
  fun produce() {
    ...
    every {
      domainEventService.get(id, PlacementAppealAccepted::class)
    } returns buildDomainEvent(
      data = PlacementAppealAcceptedFactory()
        .withBooking(
    ...
    assertThat(result.type).isEqualTo(Cas1TimelineEventType.placementAppealAccepted)
