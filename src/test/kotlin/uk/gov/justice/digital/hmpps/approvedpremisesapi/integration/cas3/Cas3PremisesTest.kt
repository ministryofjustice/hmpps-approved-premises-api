  ...
  inner class GetPremisesBedspaces {
    ...
    fun `Get Bedspaces by ID returns Not Found with correct body`() {
      givenAUser(roles = listOf(UserRole.CAS3_ASSESSOR)) { user, jwt ->
        ...
        webTestClient.get()
          .uri("/cas3/premises/$idToRequest/bedspaces")
          .header("Authorization", "Bearer $jwt")
          .exchange()
    ...
    fun `Trying to get bedspaces the user is not authorized to view should return 403`() {
      givenAUser(roles = listOf(UserRole.CAS3_ASSESSOR)) { user1, jwt1 ->
        givenAUser(roles = listOf(UserRole.CAS3_REFERRER)) { user2, jwt2 ->
