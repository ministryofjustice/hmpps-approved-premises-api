...
class Cas1BedDetailTransformerTest {
  ...
  fun `transformToApi transforms correctly`() {
    ...
    Assertions.assertThat(result.characteristics).contains(Cas1SpaceCharacteristic.arsonOffences)
    Assertions.assertThat(result.characteristics).contains(Cas1SpaceCharacteristic.hasWheelChairAccessibleBathrooms)
    Assertions.assertThat(result.characteristics).doesNotContain(Cas1SpaceCharacteristic.hasEnSuite)
  }
