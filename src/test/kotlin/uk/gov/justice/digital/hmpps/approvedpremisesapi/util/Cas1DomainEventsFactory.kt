...
import uk.gov.justice.digital.hmpps.approvedpremisesapi.jpa.entity.DEFAULT_DOMAIN_EVENT_SCHEMA_VERSION
import uk.gov.justice.digital.hmpps.approvedpremisesapi.jpa.entity.DomainEventSchemaVersion
import uk.gov.justice.digital.hmpps.approvedpremisesapi.jpa.entity.DomainEventType
import java.time.Instant
import java.util.UUID

fun createCas1DomainEventEnvelope(
  type: DomainEventType,
  objectMapper: ObjectMapper,
  schemaVersion: DomainEventSchemaVersion = DEFAULT_DOMAIN_EVENT_SCHEMA_VERSION,
  requestId: UUID = UUID.randomUUID(),
  occurredAt: Instant = Instant.now(),
): DomainEventJsonAndEnvelope {
  val expectedEnvelope = createCas1DomainEventEnvelope(
    type,
    requestId,
  ...
  val persistedJson = if (type == DomainEventType.APPROVED_PREMISES_BOOKING_CANCELLED && schemaVersion.versionNo == null) {
    removeEventDetails(
      objectMapper,
      objectMapper.writeValueAsString(expectedEnvelope),
