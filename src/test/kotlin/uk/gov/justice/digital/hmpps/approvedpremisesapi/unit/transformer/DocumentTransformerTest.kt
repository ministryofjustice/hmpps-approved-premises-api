...
class DocumentTransformerTest {
  ...
  fun `transformToApiUnfiltered transforms correctly`() {
    ...
    val result = documentTransformer.transformToApi(groupedDocuments, onlyConvictionDocuments = false)
  ...
  fun `transformToApiFiltered filters out convictions other than one specified`() {
    ...
    val result = documentTransformer.transformToApi(groupedDocuments, convictionId = 12345)
  ...
  @Test
  fun `transformToApiUnfiltered returns only conviction documents when onlyConvictionDocuments is true`() {
    val groupedDocuments = GroupedDocumentsFactory()
      .withOffenderLevelDocument(
        DocumentFactory()
          .withId(UUID.fromString("b0df5ec4-5685-4b02-8a95-91b6da80156f").toString())
    ...
    val result = documentTransformer.transformToApi(groupedDocuments, onlyConvictionDocuments = true)

    assertThat(result).hasSize(2)
    assertThat(result[0].fileName).isEqualTo("conviction_level_doc.pdf")
    assertThat(result[1].fileName).isEqualTo("conviction_level_doc_2.pdf")
  }
