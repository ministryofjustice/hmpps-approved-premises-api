        ...
        givenACas2Assessor { assessor, _ ->
          givenACas2v2PomUser { userAPrisonA, jwt ->
            givenAnOffender { offenderDetails, _ ->
              ...
              val rawResponseBody = webTestClient.get()
                .uri("/cas2v2/applications?prisonCode=${userAPrisonA.activeNomisCaseloadId}")
                .header("Authorization", "Bearer $jwt")
                .header("X-Service-Name", ServiceName.cas2.value)
                .exchange()
                .expectStatus()
        ...
        givenACas2Assessor { assessor, _ ->
          givenACas2v2PomUser { userAPrisonA, jwt ->
            givenAnOffender { offenderDetails, _ ->
              ...
              val rawResponseBody = webTestClient.get()
                .uri("/cas2v2/applications?isSubmitted=true&prisonCode=${userAPrisonA.activeNomisCaseloadId}")
                .header("Authorization", "Bearer $jwt")
                .header("X-Service-Name", ServiceName.cas2.value)
                .exchange()
                .expectStatus()
