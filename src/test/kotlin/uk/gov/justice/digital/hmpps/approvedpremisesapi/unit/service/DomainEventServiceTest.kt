...
class DomainEventServiceTest {
  ...
  private val domainEventService = DomainEventService(
    ...
    matchRequestCreatedDetailUrlTemplate = UrlTemplate("http://api/events/match-request-created/#eventId"),
  )
  ...
  @Test
  fun `getMatchRequestCreatedEvent returns event`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val applicationId = UUID.fromString("a831ead2-31ae-4907-8e1c-cad74cb9667b")
    val occurredAt = OffsetDateTime.parse("2023-02-01T14:03:00+00:00")
    val crn = "CRN"

    val data = MatchRequestCreatedEnvelope(
      id = id,
      timestamp = occurredAt.toInstant(),
      eventType = "approved-premises.match-request.created",
      eventDetails = MatchRequestCreatedFactory().produce(),
    )
    ...
    every { domainEventRespositoryMock.findByIdOrNull(id) } returns DomainEventEntityFactory()
      ...
      .withApplicationId(applicationId)
      .withCrn(crn)
      .withType(DomainEventType.APPROVED_PREMISES_MATCH_REQUEST_CREATED)
      .withData(objectMapper.writeValueAsString(data))
      .withOccurredAt(occurredAt)
      .produce()

    val event = domainEventService.getMatchRequestCreatedEvent(id)
    assertThat(event).isEqualTo(
      DomainEvent(
        id = id,
        applicationId = applicationId,
  ...
  @Test
  fun `getMatchRequestCreatedEvent returns null when event does not exist`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")

    every { domainEventRespositoryMock.findByIdOrNull(id) } returns null

    assertThat(domainEventService.getMatchRequestCreatedEvent(id)).isNull()
  }
  ...
  @Test
  fun `saveMatchRequestCreatedEvent persists event, emits event to SNS`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val applicationId = UUID.fromString("a831ead2-31ae-4907-8e1c-cad74cb9667b")
    val occurredAt = OffsetDateTime.parse("2023-02-01T14:03:00+00:00")
    val crn = "CRN"
    ...
    domainEventService.saveMatchRequestCreatedEvent(domainEventToSave)

    verify(exactly = 1) {
      domainEventRespositoryMock.save(
        match {
          it.id == domainEventToSave.id &&
            it.type == DomainEventType.APPROVED_PREMISES_MATCH_REQUEST_CREATED &&
            it.crn == domainEventToSave.crn &&
            it.occurredAt.toInstant() == domainEventToSave.occurredAt &&
            it.data == objectMapper.writeValueAsString(domainEventToSave.data)
        },
    ...
    verify(exactly = 1) {
      domainEventWorkerMock.emitEvent(
        match { snsEvent ->
          snsEvent.eventType == "approved-premises.match-request.created" &&
            snsEvent.version == 1 &&
            snsEvent.description == "An Approved Premises Match Request has been created" &&
            snsEvent.detailUrl == "http://api/events/match-request-created/$id" &&
            snsEvent.occurredAt.toInstant() == domainEventToSave.occurredAt &&
            snsEvent.additionalInformation.applicationId == applicationId &&
            snsEvent.personReference.identifiers.any { id -> id.type == "CRN" && id.value == domainEventToSave.data.eventDetails.personReference.crn } &&
            snsEvent.personReference.identifiers.any { id -> id.type == "NOMS" && id.value == domainEventToSave.data.eventDetails.personReference.noms }
  ...
  @Test
  fun `saveMatchRequestCreatedEvent does not emit event to SNS if event fails to persist to database`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val applicationId = UUID.fromString("a831ead2-31ae-4907-8e1c-cad74cb9667b")
    val occurredAt = OffsetDateTime.parse("2023-02-01T14:03:00+00:00")
    val crn = "CRN"
