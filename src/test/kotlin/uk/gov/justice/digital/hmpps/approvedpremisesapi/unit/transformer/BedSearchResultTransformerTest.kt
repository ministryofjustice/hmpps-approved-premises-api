...
import uk.gov.justice.digital.hmpps.approvedpremisesapi.factory.ApprovedPremisesBedSearchResultFactory
...
class BedSearchResultTransformerTest {
  ...
  @Test
  fun `transformDomainToApi transforms correctly for Approved Premises results`() {
    ...
    val domainResults = listOf(
      ApprovedPremisesBedSearchResultFactory()
    ...
    result.results.forEachIndexed { index, it ->
      ...
      assertThat(domainResult.distance.toBigDecimal()).isEqualTo((it as ApiApprovedPremisesBedSearchResult).distanceMiles)
    }
  }
