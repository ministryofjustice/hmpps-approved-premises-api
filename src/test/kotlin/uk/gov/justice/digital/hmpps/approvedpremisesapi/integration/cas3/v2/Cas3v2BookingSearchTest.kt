...
class Cas3v2BookingSearchTest : IntegrationTestBase() {
  ...
  @CsvSource(
    value = [
      "PERSON_NAME,ascending,departed",
      "PERSON_NAME,descending,departed",
    ],
  )
    ...
    givenAUser { userEntity, jwt ->
      givenSomeOffenders { offenderSequence ->
        ...
        for (page in 0..<totalPages) {
          ...
          val apiUri = "/cas3/v2/bookings/search?sortOrder=$sortOrder&sortField=${bookingSearchSort.value}&status=departed&page=${page + 1}"
          webTestClient.get()
            .uri(apiUri)
  ...
  @CsvSource(
    value = [
      "PERSON_CRN,ascending",
      "BOOKING_START_DATE,ascending",
      "BOOKING_END_DATE,ascending",
      "BOOKING_CREATED_AT,ascending",
      "PERSON_CRN,descending",
      "BOOKING_START_DATE,descending",
      "BOOKING_END_DATE,descending",
      "BOOKING_CREATED_AT,descending",
    ],
  )
  fun `Searching for Temporary Accommodation bookings with pagination returns 200 with correct subset of results`(
    ...
    givenAUser { userEntity, jwt ->
      givenAnOffender { offenderDetails, _ ->
        ...
        webTestClient.get()
          .uri("/cas3/v2/bookings/search?sortOrder=$sortDirection&sortField=${bookingSearchSort.value}&page=1")
          .header("Authorization", "Bearer $jwt")
          .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
          .exchange()
          .expectStatus()
        ...
        webTestClient.get()
          .uri("/cas3/v2/bookings/search?sortOrder=$sortDirection&sortField=${bookingSearchSort.value}&page=2")
          .header("Authorization", "Bearer $jwt")
          .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
          .exchange()
          .expectStatus()
  ...
  fun `Results are ordered by the start date when multiple booking have the same arrival date when the query parameters are supplied with Pagination`(
    ...
    givenAUser { userEntity, jwt ->
      ...
      webTestClient.get()
        .uri("/cas3/v2/bookings/search?sortOrder=$sortDirection&sortField=startDate&page=1&status=provisional")
        .header("Authorization", "Bearer $jwt")
        .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
        .exchange()
        .expectStatus()
      ...
      webTestClient.get()
        .uri("/cas3/v2/bookings/search?sortOrder=$sortDirection&sortField=startDate&page=2&status=provisional")
        .header("Authorization", "Bearer $jwt")
        .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
        .exchange()
        .expectStatus()
  ...
  fun `Results are ordered by the end date when multiple booking have the same arrival date when the query parameters are supplied with Pagination`(
    ...
    givenAUser { userEntity, jwt ->
      ...
      webTestClient.get()
        .uri("/cas3/v2/bookings/search?sortOrder=$sortDirection&sortField=endDate&page=1&status=provisional")
        .header("Authorization", "Bearer $jwt")
        .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
        .exchange()
        .expectStatus()
      ...
      webTestClient.get()
        .uri("/cas3/v2/bookings/search?sortOrder=$sortDirection&sortField=endDate&page=2&status=provisional")
        .header("Authorization", "Bearer $jwt")
        .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
        .exchange()
        .expectStatus()
  ...
  fun `Results are ordered by the person crn and sorted descending order when the query parameters are supplied with Pagination`() {
    givenAUser { userEntity, jwt ->
      givenAnOffender { offenderDetails, _ ->
        ...
        webTestClient.get()
          .uri("/cas3/v2/bookings/search?sortOrder=descending&sortField=crn&page=1&status=provisional")
          .header("Authorization", "Bearer $jwt")
          .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
          .exchange()
          .expectStatus()
  ...
  fun `Get all results ordered by the person crn in descending order when the query parameters supplied without Pagination`() {
    givenAUser { userEntity, jwt ->
      givenAnOffender { offenderDetails, _ ->
        ...
        callApiAndAssertResponse("/cas3/v2/bookings/search?sortOrder=descending&sortField=crn&status=provisional", jwt, expectedResponse, true)
      }
    }
  }
  ...
  fun `Results are ordered by the person name when the query parameters are supplied with Pagination`(
    ...
    givenAUser { userEntity, jwt ->
      ...
      webTestClient.get()
        .uri("/cas3/v2/bookings/search?sortOrder=$sortDirection&sortField=name&page=1&status=provisional")
        .header("Authorization", "Bearer $jwt")
        .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
        .exchange()
        .expectStatus()
      ...
      webTestClient.get()
        .uri("/cas3/v2/bookings/search?sortOrder=$sortDirection&sortField=name&page=2&status=provisional")
        .header("Authorization", "Bearer $jwt")
        .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
        .exchange()
        .expectStatus()
  ...
  fun `No Results returned when searching for cancelled booking status and all existing bookings are confirmed`() {
    givenAUser { userEntity, jwt ->
      givenAnOffender { offenderDetails, _ ->
        ...
        webTestClient.get()
          .uri("/cas3/v2/bookings/search?sortOrder=descending&sortField=crn&page=1&status=cancelled")
          .header("Authorization", "Bearer $jwt")
          .header("X-Service-Name", ServiceName.temporaryAccommodation.value)
          .exchange()
          .expectStatus()
  ...
  private fun getSortedBooking(bookings: MutableList<Cas3BookingEntity>, sortBy: Cas3BookingSearchSortField, sortOrder: SortOrder) = when (sortBy) {
    Cas3BookingSearchSortField.BOOKING_END_DATE -> sortBookings(Cas3BookingEntity::departureDate, bookings, sortOrder)
    Cas3BookingSearchSortField.BOOKING_START_DATE -> sortBookings(Cas3BookingEntity::arrivalDate, bookings, sortOrder)
    Cas3BookingSearchSortField.PERSON_CRN -> sortBookings(Cas3BookingEntity::crn, bookings, sortOrder)
    else -> sortBookings(Cas3BookingEntity::createdAt, bookings, sortOrder)
  }

  fun <T : Comparable<T>> sortBookings(fn: Cas3BookingEntity.() -> T, bookings: MutableList<Cas3BookingEntity>, sortOrder: SortOrder) = when (sortOrder) {
