...
class Cas3PremisesTest : Cas3IntegrationTestBase() {
  ...
  inner class GetPremisesSummary {
    ...
    fun `Get all Premises returns OK with correct body`() {
      givenAUser(roles = listOf(UserRole.CAS3_ASSESSOR)) { user, jwt ->
        val expectedPremises = givenATemporaryAccommodationPremises(region = user.probationRegion)
        ...
        webTestClient.get()
          ...
          .jsonPath("$[0].addressLine1").isEqualTo(expectedPremises.addressLine1)
          .jsonPath("$[0].addressLine2").isEqualTo(expectedPremises.addressLine2)
          .jsonPath("$[0].postcode").isEqualTo(expectedPremises.postcode)
          .jsonPath("$[0].status").isEqualTo(expectedPremises.status)
    ...
    @Test
    fun `Get all Premises returns a bedspace count as expected when bedspaces are active`() {
      givenAUser { user, jwt ->
        val expectedPremises = givenATemporaryAccommodationPremises(region = user.probationRegion)
