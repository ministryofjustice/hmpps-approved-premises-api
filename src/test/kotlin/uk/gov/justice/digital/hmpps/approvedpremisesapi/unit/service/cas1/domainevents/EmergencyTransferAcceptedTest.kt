...
import uk.gov.justice.digital.hmpps.approvedpremisesapi.service.cas1.domainevent.EmergencyTransferCreatedTimelineFactory
import java.time.LocalDate
import java.util.UUID

@ExtendWith(MockKExtension::class)
class EmergencyTransferAcceptedTest {
  @MockK
  lateinit var domainEventService: Cas1DomainEventService

  @InjectMockKs
  lateinit var service: EmergencyTransferCreatedTimelineFactory
  ...
  fun produce() {
    ...
    every {
      domainEventService.get(id, EmergencyTransferCreated::class)
    } returns buildDomainEvent(
      data = EmergencyTransferCreatedFactory()
        .withFrom(
    ...
    assertThat(result.type).isEqualTo(Cas1TimelineEventType.emergencyTransferCreated)
