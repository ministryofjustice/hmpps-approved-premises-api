...
class Cas1ApplicationTest : IntegrationTestBase() {
    ...
    fun `Get single non LAO application returns 200 with correct body`() {
      givenAUser { userEntity, jwt ->
        givenAnOffender { offenderDetails, _ ->
          ...
          val rawResponseBody = webTestClient.get()
            .uri("/cas1/applications/${applicationEntity.id}")
            .header("Authorization", "Bearer $jwt")
            .exchange()
            .expectStatus()
            .isOk
    ...
    fun `Get single online application returns 200 non-upgradable outdated application marked as such`() {
      givenAUser { userEntity, jwt ->
        givenAnOffender { offenderDetails, _ ->
          ...
          val rawResponseBody = webTestClient.get()
            .uri("/cas1/applications/${nonUpgradableApplicationEntity.id}")
            .header("Authorization", "Bearer $jwt")
            .exchange()
            .expectStatus()
            .isOk
    ...
    fun `Get single offline application returns 200 with correct body`(role: UserRole) {
      givenAUser(roles = listOf(role)) { userEntity, jwt ->
        givenAnOffender { offenderDetails, _ ->
          ...
          val rawResponseBody = webTestClient.get()
            .uri("/cas1/applications/${offlineApplicationEntity.id}")
            .header("Authorization", "Bearer $jwt")
            .exchange()
            .expectStatus()
            .isOk
  ...
  inner class SubmitApplication {
    ...
    @Test
    fun `When several concurrent submit application requests occur, one is successful and others return 400 without persisting domain events`() {
      givenAUser { submittingUser, jwt ->
        givenAUser(
          roles = listOf(UserRole.CAS1_ASSESSOR),
        ) { _, _ ->
  ...
  inner class CreateApplicationNote {
    ...
    fun `post ApplicationTimelineNote without JWT returns 401`() {
      webTestClient.post()
        .uri("/cas1/applications/${UUID.randomUUID()}/notes")
        .exchange()
        .expectStatus()
        .isUnauthorized
    }
    ...
    fun `post ApplicationTimelineNote with JWT returns 200`() {
      givenAUser(roles = listOf(UserRole.CAS1_CRU_MEMBER)) { user, jwt ->
        ...
        webTestClient.post()
          .uri("/cas1/applications/$applicationId/notes")
          .header("Authorization", "Bearer $jwt")
          .bodyValue(
            body,
          )
  ...
  inner class WithdrawApplication {
    ...
    fun `Withdraw Application without JWT returns 401`() {
      webTestClient.post()
        .uri("/cas1/applications/${UUID.randomUUID()}/withdrawal")
        .exchange()
        .expectStatus()
        .isUnauthorized
    }
  ...
  inner class GetRequestsForPlacements {
    ...
    fun `Get all Requests for Placement for an application without a JWT returns a 401 response`() {
      webTestClient.get()
        .uri("/cas1/applications/${UUID.randomUUID()}/requests-for-placement")
        .exchange()
        .expectStatus()
        .isUnauthorized
    }
      ...
      givenAUser { user, jwt ->
        givenAnAssessmentForApprovedPremises(
          ...
          val requestForPlacements = webTestClient.get()
            .uri("/cas1/applications/${application.id}/requests-for-placement")
            .header("Authorization", "Bearer $jwt")
            .exchange()
            .expectStatus()
            .isOk
          ...
          assertThat(requestForPlacements[0].requestedPlacementPeriod).isEqualTo(Cas1RequestedPlacementPeriod(arrival = LocalDate.now(), duration = 2, arrivalFlexible = false))
          ...
          assertThat(requestForPlacements[2].requestedPlacementPeriod).isEqualTo(Cas1RequestedPlacementPeriod(arrival = LocalDate.now(), duration = 12, arrivalFlexible = null))
          assertThat(requestForPlacements[2].authorisedPlacementPeriod).isEqualTo(Cas1RequestedPlacementPeriod(arrival = LocalDate.now(), duration = 12, arrivalFlexible = null))
