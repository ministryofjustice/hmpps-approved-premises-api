...
class Cas3PremisesServiceTest {
  ...
  inner class GetArchiveHistory {
    ...
    fun `When getArchiveHistory returns Success with empty list of histories`() {
      ...
      every { mockCas3DomainEventService.getBedspaceArchiveOrUnarchiveEvents(bedspaceId) } returns emptyList()
    ...
    fun `When getArchiveHistory returns Success with list of events`() {
      ...
      every { mockCas3DomainEventService.getBedspaceArchiveOrUnarchiveEvents(bedspaceId) } returns
        listOf(
          domainEventTomorrow,
          domainEventToday,
  ...
  inner class ArchiveBedspace {
    ...
    fun `When archive a bedspace returns Success with correct result when validation passed`() {
      ...
      every { bedRepositoryMock.save(any()) } returns bedspace
      every { mockCas3DomainEventService.saveBedspaceArchiveEvent(bedspace) } returns Unit
  ...
  inner class UnarchiveBedspace {
    ...
    fun `unarchiveBedspace returns Success when bedspace is successfully unarchived`() {
      ...
      every { bedRepositoryMock.save(any()) } returns updatedBedspace
      every { mockCas3DomainEventService.saveBedspaceUnarchiveEvent(match { it.id == updatedBedspace.id }, currentStartDate, currentEndDate) } returns Unit
    ...
    fun `unarchiveBedspace allows restart date exactly 7 days in the future`() {
      ...
      every { mockCas3DomainEventService.saveBedspaceUnarchiveEvent(match { it.id == archivedBedspace.id }, currentStartDate, currentEndDate) } returns Unit
      every { bedRepositoryMock.save(any()) } returns updatedBedspace
  ...
  inner class CancelUnarchiveBedspace {
    ...
    fun `cancelUnarchiveBedspace returns Success when scheduled unarchive is cancelled`() {
      ...
      every { bedRepositoryMock.save(any()) } returns updatedBedspace
      every { mockCas3DomainEventService.getLastBedspaceUnarchiveEventDetails(bedspaceId) } returns eventDetails
    ...
    fun `cancelUnarchiveBedspace returns FieldValidationError when bedspace unarchive event does not exist`() {
      ...
      every { mockCas3DomainEventService.getLastBedspaceUnarchiveEventDetails(bedspaceId) } returns null
      every { bedRepositoryMock.findById(bedspaceId) } returns Optional.of(scheduledToUnarchiveBedspace)
