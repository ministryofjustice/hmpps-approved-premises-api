...
class Cas1BedDetailTest : InitialiseDatabasePerClassTestBase() {
  ...
  fun `Getting a bed for a premises returns the bed`() {
    givenAUser(roles = listOf(UserRole.CAS1_FUTURE_MANAGER)) { user, jwt ->
      ...
      Assertions.assertThat(response.characteristics.contains(Cas1SpaceCharacteristic.hasEnSuite))
      Assertions.assertThat(response.characteristics.contains(Cas1SpaceCharacteristic.isGroundFloor))
      Assertions.assertThat(!response.characteristics.contains(Cas1SpaceCharacteristic.hasCrib7Bedding))
    }
  }
}
