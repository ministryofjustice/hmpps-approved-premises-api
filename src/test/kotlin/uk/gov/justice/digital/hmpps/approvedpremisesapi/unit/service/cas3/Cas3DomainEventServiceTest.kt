...
class Cas3DomainEventServiceTest {
  ...
  fun `saveBedspaceArchiveEvent saves event but does not emit it`() {
    ...
    every { cas3DomainEventBuilderMock.getBedspaceArchiveEvent(eq(bedspace), eq(premises.id), eq(user)) } returns domainEvent
    every { domainEventRepositoryMock.save(any()) } returns null
    every { userService.getUserForRequest() } returns user
    cas3DomainEventService.saveBedspaceArchiveEvent(bedspace, premises.id)
