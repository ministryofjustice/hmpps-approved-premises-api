...
class Cas3v2VoidBedspaceTest : Cas3IntegrationTestBase() {
  ...
  inner class CreateVoidBedspace {
    ...
    @Test
    fun `Create Void Bedspaces on Temporary Accommodation premises returns 404 Bad Request if the bed ID does not reference a bed on the premises`() {
      givenAUser(roles = listOf(UserRole.CAS3_ASSESSOR)) { user, jwt ->
        val premises = givenACas3Premises(user.probationRegion)
        val reason = cas3VoidBedspaceReasonEntityFactory.produceAndPersist()
    ...
    fun `Create Void Bedspace on a Temporary Accommodation premises returns 409 Conflict when a void bedspace for the same bed overlaps`() {
      givenAUser(roles = listOf(UserRole.CAS3_ASSESSOR)) { user, jwt ->
        ...
        doPostRequest(jwt, premises.id, voidBedspace)
          ...
          .jsonPath("status").isEqualTo(409)
          .jsonPath("detail")
          .isEqualTo("A Lost Bed already exists for dates from $startDate to $endDate which overlaps with the desired dates: ${existingVoidBedspace.id}")
      }
    }
