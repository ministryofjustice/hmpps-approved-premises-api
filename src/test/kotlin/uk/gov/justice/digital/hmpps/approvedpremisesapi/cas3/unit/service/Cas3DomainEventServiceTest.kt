...
class Cas3DomainEventServiceTest {
  ...
  fun `saveBookingConfirmedEvent persists event, emits event to SNS`() {
    ...
    every { cas3DomainEventBuilderMock.getBookingConfirmedDomainEvent(any(BookingEntity::class), user) } returns domainEventToSave
  ...
  @Test
  fun `saveBookingConfirmedEvent with cas3 booking persists event, emits event to SNS`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val mockHmppsTopic = mockk<HmppsTopic>()
    val domainEventToSave = DomainEvent(
      id = id,
    ...
    assertAll(
      ...
      { assertThat(deserializedMessage.personReference.identifiers.any {
  ...
  @Test
  fun `saveBookingConfirmedEvent with cas3 booking persists event, but does not emit event to SNS when event is disabled`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val mockHmppsTopic = mockk<HmppsTopic>()
    val domainEventToSave = DomainEvent(
      id = id,
  ...
  @Test
  fun `saveBookingConfirmedEvent with cas3 booking does not emit event to SNS if event fails to persist to database`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val mockHmppsTopic = mockk<HmppsTopic>()
    val domainEventToSave = DomainEvent(
      id = id,
  ...
  fun `savePersonDepartedEvent persists event, emits event to SNS`() {
    ...
    every { cas3DomainEventBuilderMock.getPersonDepartedDomainEvent(any(BookingEntity::class), user) } returns domainEventToSave
  ...
  @Test
  fun `savePersonDepartedEvent for cas3 booking persists event, emits event to SNS`() {
    val id = UUID.randomUUID()
    val mockHmppsTopic = mockk<HmppsTopic>()
  ...
  @Test
  fun `savePersonDepartedEvent for cas3 booking persists event, but does not emit event to SNS when event is disabled`() {
    val id = UUID.randomUUID()
    val mockHmppsTopic = mockk<HmppsTopic>()
  ...
  @Test
  fun `savePersonDepartedEvent for cas3 booking does not emit event to SNS if event fails to persist to database`() {
    val id = UUID.randomUUID()
    val mockHmppsTopic = mockk<HmppsTopic>()
  ...
  fun `should savePersonDepartureUpdatedEvent persists given event into DB and emits event to SNS`() {
    ...
    every { domainEventRepositoryMock.save(any()) } answers { it.invocation.args[0] as DomainEventEntity }
    every { hmppsQueueServiceMock.findByTopicId("domainevents") } returns mockHmppsTopic
    every { cas3DomainEventBuilderMock.buildDepartureUpdatedDomainEvent(any(BookingEntity::class), user) } returns domainEventToSave
    every { mockHmppsTopic.arn } returns "arn:aws:sns:eu-west-2:000000000000:domain-events"
    every { mockHmppsTopic.snsClient.publish(any<PublishRequest>()) } returns CompletableFuture.completedFuture(PublishResponse.builder().build())
  ...
  @Test
  fun `should savePersonDepartureUpdatedEvent for cas3 booking persists given event into DB and emits event to SNS`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val domainEventToSave = createCAS3DepartureUpdatedDomainEvent(
      id,
      applicationId = UUID.fromString("a831ead2-31ae-4907-8e1c-cad74cb9667b"),
  ...
  @Test
  fun `should not emit SNS event for cas3 booking when savePersonDepartureUpdatedEvent persists event fail to store to DB`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val domainEventToSave = createCAS3DepartureUpdatedDomainEvent(
      id,
      applicationId = UUID.fromString("a831ead2-31ae-4907-8e1c-cad74cb9667b"),
  ...
  @Test
  fun `Should throw error for cas3 booking when savePersonDepartureUpdatedEvent fail  to publish and save event in DB`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val domainEventToSave = createCAS3DepartureUpdatedDomainEvent(
      id,
      applicationId = UUID.fromString("a831ead2-31ae-4907-8e1c-cad74cb9667b"),
  ...
  @Test
  fun `should savePersonDepartureUpdatedEvent persists given event for cas3 booking but does not emit event to SNS when event is disabled`() {
    val id = UUID.fromString("c3b98c67-065a-408d-abea-a252f1d70981")
    val domainEventToSave = createCAS3DepartureUpdatedDomainEvent(
      id,
      applicationId = UUID.fromString("a831ead2-31ae-4907-8e1c-cad74cb9667b"),
