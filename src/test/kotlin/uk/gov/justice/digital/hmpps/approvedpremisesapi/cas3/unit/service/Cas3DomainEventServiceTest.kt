...
class Cas3DomainEventServiceTest {
  ...
  fun `getBedspaceDomainEvents returns list of bedspace archive events`() {
    ...
    val dataToday = createBedspaceArchiveEvent(premisesId = premisesId, bedspaceId = bedspaceId, userId = userId, null, endDate = endDateToday, transactionId = UUID.randomUUID())
    ...
    val dataYesterday = createBedspaceUnarchiveEvent(premisesId = premisesId, bedspaceId = bedspaceId, userId = userId, newStartDate = newStartDateYesterday, transactionId = UUID.randomUUID())
    ...
    val dataTomorrow = createBedspaceArchiveEvent(premisesId = premisesId, bedspaceId = bedspaceId, userId = userId, null, endDate = endDateTomorrow, transactionId = UUID.randomUUID())
  ...
  fun `savePremisesUnarchiveEvent saves event but does not emit it`() {
    ...
    cas3DomainEventService.savePremisesUnarchiveEvent(premises, currentStartDate, newStartDate, currentEndDate, domainEventTransactionId)
  ...
  fun `savePersonArrivedUpdatedEvent persists updated arrival event and emits event to SNS`() {
    ...
    every { domainEventRepositoryMock.save(any()) } answers { it.invocation.args[0] as DomainEventEntity }
    every { hmppsQueueServiceMock.findByTopicId("domainevents") } returns mockHmppsTopic
    every { cas3DomainEventBuilderMock.buildPersonArrivedUpdatedDomainEvent(any(Cas3BookingEntity::class), user) } returns domainEventToSave
    every { mockHmppsTopic.arn } returns "arn:aws:sns:eu-west-2:000000000000:domain-events"
    every { mockHmppsTopic.snsClient.publish(any<PublishRequest>()) } returns CompletableFuture.completedFuture(PublishResponse.builder().build())
  ...
  private fun createPremisesEntity(probationRegion: ProbationRegionEntity): Cas3PremisesEntity {
  ...
  private fun createBedspaceEntity(premises: Cas3PremisesEntity): Cas3BedspacesEntity = Cas3BedspaceEntityFactory()
    .withEndDate(null)
    .withPremises(premises)
    .produce()

  private fun createTemporaryAccommodationPremisesBookingEntity(): Cas3BookingEntity {
    val probationRegion = ProbationRegionEntityFactory()
      .withYieldedApArea { ApAreaEntityFactory().produce() }
      .produce()
