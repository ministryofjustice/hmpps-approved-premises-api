...
-- TODO besscerule added the default of HDC
...
SELECT a.id,
       a.crn,
       a.noms_number,
       a.created_by_cas2_user_id::text AS created_by_user_id,
       cu.name AS name,
       a.created_at,
       a.submitted_at,
       ...
       cu2.name as allocated_pom_name,
...
--
-- -- must have either cas2 or external user
-- ALTER TABLE cas_2_application_notes
--     ADD CONSTRAINT has_user
--         CHECK
--             ((created_by_cas2_user_id is not null and created_by_external_user_id is null)
--             OR
--              (created_by_cas2_user_id is null and created_by_external_user_id is not null));