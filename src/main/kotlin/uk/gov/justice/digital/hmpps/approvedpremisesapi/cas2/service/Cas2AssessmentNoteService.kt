...
class Cas2AssessmentNoteService(
  ...
  private fun sendEmailToReferrer(savedNote: Cas2ApplicationNoteEntity) {
    ...
    if (email != null) {
      ...
      val msg = "Email not found for User ${savedNote.application.createdByCas2User.id}. Unable to send email for Note ${savedNote.id} on Application ${savedNote.application.id}"
      log.error(msg)
