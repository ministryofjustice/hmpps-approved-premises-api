...
class Cas3PremisesService(
  ...
  fun cancelScheduledArchivePremises(
    ...
    if (premisesArchiveDomainEventData.eventDetails.endDate <= LocalDate.now()) {
  ...
  fun canArchiveBedspaceInFuture(premisesId: UUID, bedspaceId: UUID): CasResult<Cas3ValidationResult?> {
    val result = getBedspace(premisesId, bedspaceId)

    if (result is Cas3FieldValidationError) {
      return Cas3FieldValidationError(
        validationMessages = result.validationMessages,
      )
    } else {
      val bedEntity = extractEntityFromCasResult(result)

      val threeMonthsFromToday = LocalDate.now().plusMonths(MAX_MONTHS_ARCHIVE_PREMISES_IN_FUTURE)
      val blockingDate= mutableListOf<LocalDate>();
      ...
      overlapBookings.map {
        val bookingTurnaround = workingDayService.addWorkingDays(it.departureDate, it.turnaround?.workingDayCount ?: 0)
        if (bookingTurnaround >= threeMonthsFromToday) {
          blockingDate += bookingTurnaround
        }
      }
      ...
      if (overlappingVoid != null) {
        blockingDate += overlappingVoid.endDate
      }

      if (blockingDate.isEmpty()) {
        return CasResult.Success(null)
      } else {
        return CasResult.Success(Cas3ValidationResult(bedspaceId, bedEntity.room.name, blockingDate.max()))
      }
    }
  }
}
