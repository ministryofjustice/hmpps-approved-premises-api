...
class Cas2UserService(
  ...
  fun getUserDtoForRequest(): CasResult<Cas2UserDto> {
    ...
    return CasResult.Success(
      Cas2UserDto(
        ...
        deliusUserInfo = if (typedUser is TypedUser.Delius) {
  ...
  private fun getTypedUserForRequest(): TypedUser {
  ...
  private fun getEntityForNomisUser(username: String, jwt: String): TypedUser.Nomis {
    ...
    if (existingUser != null) {
      if (existingUser.email != nomisUserDetails.primaryEmail || existingUser.activeNomisCaseloadId != nomisUserDetails.activeCaseloadId) {
        ...
        return TypedUser.Nomis(cas2UserRepository.save(existingUser))
      }

      return TypedUser.Nomis(existingUser)
    }
    ...
    return TypedUser.Nomis(getExistingUser(username, Cas2UserType.NOMIS)!!)
  }

  private fun getEntityForDeliusUser(username: String): TypedUser.Delius {
    ...
    if (existingUser != null) {
      ...
      if (deliusUser.email != existingUser.email || teamsDiffer) {
        ...
        return TypedUser.Delius(cas2UserRepository.save(existingUser), deliusUser)
      }

      return TypedUser.Delius(existingUser, deliusUser)
    }
    ...
    return TypedUser.Delius(getExistingUser(username, Cas2UserType.DELIUS)!!, deliusUser)
  }

  private fun getEntityForExternalUser(username: String, jwt: String): TypedUser.External {
    val existingUser = getExistingUser(username, Cas2UserType.EXTERNAL)
    if (existingUser != null) return TypedUser.External(existingUser)
    ...
    return TypedUser.External(getExistingUser(username, Cas2UserType.EXTERNAL)!!)
  }
}

sealed interface TypedUser {
  val userEntity: Cas2UserEntity

  data class Nomis(override val userEntity: Cas2UserEntity) : TypedUser
  data class Delius(override val userEntity: Cas2UserEntity, val deliusUser: StaffDetail) : TypedUser
  data class External(override val userEntity: Cas2UserEntity) : TypedUser
}
