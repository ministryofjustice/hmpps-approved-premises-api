...
interface TransitionalAccommodationReferralReportRepository : JpaRepository<BookingEntity, UUID> {
  @Query(
    ...
    SELECT cast(a.id as text) AS assessmentId,
      ...
      pr_probation_region.name as previousReferralProbationRegionName,
      pr_pdu.name as previousReferralPduName
    ...
    LEFT JOIN probation_regions pr_probation_region ON pr_probation_region.id = taa.previous_referral_probation_region_id
    LEFT JOIN probation_delivery_units pr_pdu ON pr_pdu.id = taa.previous_referral_probation_delivery_unit_id
