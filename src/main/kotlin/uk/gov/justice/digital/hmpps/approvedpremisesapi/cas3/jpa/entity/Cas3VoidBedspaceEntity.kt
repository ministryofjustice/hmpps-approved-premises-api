...
interface Cas3VoidBedspacesRepository : JpaRepository<Cas3VoidBedspaceEntity, UUID> {
  ...
  @Query("SELECT lb FROM Cas3VoidBedspaceEntity lb WHERE lb.startDate <= :endDate AND lb.endDate >= :startDate AND lb.bedspace = :bedspace AND lb.cancellationDate is NULL")
