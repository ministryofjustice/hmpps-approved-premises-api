...
data class RequestForPlacementAssessed(
  ...
  @Schema(example = "7", required = true, description = "Duration may be null if decision is rejected and the applicant didn't define a duration")
  ...
  enum class Decision(@get:JsonValue val value: String) {
    ...
    companion object {
      @JvmStatic
      @JsonCreator
      fun forValue(value: String): Decision = values().first { it.value == value }
    }
  }
}
