...
class Cas1OffenderService(
  ...
  fun getRiskTier(crn: String): CasResult<RiskTier> =
    when (val tierResponse = hmppsTierApiClient.getTier(crn)) {
        is ClientResult.Success -> CasResult.Success(RiskTier(
  ...
  fun getCas1PersonSummaryInfoResult(crn: String, cas1LaoStrategy: LaoStrategy): CasResult<PersonSummaryInfoResult>{
    val personSummaryInfoResult = offenderService.getPersonSummaryInfoResults(setOf(crn), cas1LaoStrategy).first()
    if (personSummaryInfoResult is PersonSummaryInfoResult.NotFound || personSummaryInfoResult is PersonSummaryInfoResult.Unknown) {
      return CasResult.NotFound(crn, "Offender")
    }
    return CasResult.Success(personSummaryInfoResult)
  }
}
