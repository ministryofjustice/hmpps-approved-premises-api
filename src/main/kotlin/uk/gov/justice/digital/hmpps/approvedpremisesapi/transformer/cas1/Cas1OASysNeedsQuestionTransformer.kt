...
import uk.gov.justice.digital.hmpps.approvedpremisesapi.model.oasyscontext.Health
...
class Cas1OASysNeedsQuestionTransformer(val featureFlagService: FeatureFlagService) {
  ...
  fun transformToOASysQuestion(needsDetails: NeedsDetails?, health: Health?, includeOptionalSections: List<Int>) = toQuestionState(needsDetails, health)
  ...
  private fun toQuestionState(needsDetails: NeedsDetails?, health: Health?) = listOf(
    QuestionState(
      sectionNumber = 3,
    ...
    QuestionState(
      ...
      answer = if (health?.generalHealth == true) health.generalHealthSpecify else null,
      optional = true,
      linkedToHarm = null,
