...
class Cas2v2ApplicationStatusTransformer {
  ...
  fun transformStatusDetailListToDetailItemList(statusDetailsList: List<Cas2v2PersistedApplicationStatusDetail>): List<Cas2v2StatusDetail> {
    return statusDetailsList.map { status -> transformStatusDetailToStatusDetailItem(status) }
  }

  fun transformStatusDetailToStatusDetailItem(statusDetail: Cas2v2PersistedApplicationStatusDetail): Cas2v2StatusDetail {
    return Cas2v2StatusDetail(
      name = statusDetail.name,
      label = statusDetail.label,
    )
  }
