  ...
  companion object {
    private const val SELECT_DATASET_QUERY = """
          SELECT csb.id AS placement_id,
                 ...
                 csb.cancellation_occurred_at AS placement_withdrawn_date_time,
                 plr.withdrawal_reason AS placement_withdrawn_reason,
                 STRING_AGG(c.property_name, ',') OVER (PARTITION BY csb.id) AS criteria,
                 matching.*
          ...
          LEFT JOIN departures d ON d.id = dr.id
          LEFT JOIN move_on_categories moc ON moc.id = d.id
          LEFT JOIN placement_requests plr ON plr.id = csb.placement_request_id
          LEFT JOIN cas1_space_bookings_criteria criteria on criteria.space_booking_id = csb.id
