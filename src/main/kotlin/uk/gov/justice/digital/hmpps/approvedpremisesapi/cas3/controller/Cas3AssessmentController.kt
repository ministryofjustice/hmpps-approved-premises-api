...
class Cas3AssessmentController(
  ...
  @PostMapping("/assessments/{assessmentId}/allocations")
  @Transactional
  fun reallocateAssessment(@PathVariable assessmentId: UUID): ResponseEntity<Unit> {
    val user = userService.getUserForRequest()

    ensureEntityFromCasResultIsSuccess(
      cas3AssessmentService.reallocateAssessment(user, assessmentId),
    )
