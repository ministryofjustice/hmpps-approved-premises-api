...
class Cas3MigrateNewBedspaceModelDataJob(
  ...
  private fun generateCas3BedspacesCharacteristics(): List<Cas3BedspaceCharacteristicEntity> = characteristicRepository.findAllCharacteristicsReferenceData(
    modelScope = "room",
    serviceScope = ServiceName.temporaryAccommodation.value,
  ).map {
    Cas3BedspaceCharacteristicEntity(
      id = it.id,
      code = it.propertyName,
      name = it.name,
      isActive = it.isActive,
    )
  }

  private fun generateCas3PremisesCharacteristics() = characteristicRepository.findAllCharacteristicsReferenceData(
    modelScope = "premises",
    serviceScope = ServiceName.temporaryAccommodation.value,
  ).map {
  ...
  private fun generateCas3BedspacesCharacteristicsMappingBatch(cas3BedspacesBatch: List<Cas3BedspacesEntity>) = characteristicRepository.findBedspaceCharacteristicsMappingsByBedIds(cas3BedspacesBatch.map { it.id })
    .map { Cas3BedspaceCharacteristicMappingEntity(id = it) }
  ...
  private fun generateCas3PremisesCharacteristicsMappingBatch(cas3PremisesBatch: List<Cas3PremisesEntity>) = characteristicRepository.findPremisesCharacteristicsMappingsByPremiseIds(cas3PremisesBatch.map { it.id })
    .map { Cas3PremisesCharacteristicMappingEntity(id = it) }
}
...
@Repository
interface Cas3BedspaceCharacteristicMappingRepository : JpaRepository<Cas3BedspaceCharacteristicMappingEntity, UUID>

@Repository
interface Cas3PremisesCharacteristicMappingRepository : JpaRepository<Cas3PremisesCharacteristicMappingEntity, UUID>

@Entity
@Table(name = "cas3_bedspace_characteristic_mappings")
@org.hibernate.annotations.Cache(usage = CacheConcurrencyStrategy.READ_ONLY)
data class Cas3BedspaceCharacteristicMappingEntity(
  @EmbeddedId
  val id: Cas3BedspaceCharacteristicMappingId,
)

@Entity
@Table(name = "cas3_premises_characteristic_mappings")
@org.hibernate.annotations.Cache(usage = CacheConcurrencyStrategy.READ_ONLY)
data class Cas3PremisesCharacteristicMappingEntity(
  @EmbeddedId
  val id: Cas3PremisesCharacteristicMappingId,
)

@Embeddable
data class Cas3BedspaceCharacteristicMappingId(
...
@Embeddable
data class Cas3PremisesCharacteristicMappingId(
