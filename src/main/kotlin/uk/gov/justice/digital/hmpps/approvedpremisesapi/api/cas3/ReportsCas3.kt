...
interface ReportsCas3 {
    ...
    fun reportsReportNameGet(@Parameter(description = "Name of the report to download", required = true, schema = Schema(allowableValues = ["referral", "booking", "bedUsage", "bedOccupancy", "futureBookings", "futureBookingsCsv", "bookingGap"])) @PathVariable("reportName") reportName: Cas3ReportType, @RequestParam(value = "startDate", required = true) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) startDate: java.time.LocalDate, @RequestParam(value = "endDate", required = true) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) endDate: java.time.LocalDate, @RequestParam(value = "probationRegionId", required = false) probationRegionId: java.util.UUID?): ResponseEntity<org.springframework.web.servlet.mvc.method.annotation.StreamingResponseBody> {
