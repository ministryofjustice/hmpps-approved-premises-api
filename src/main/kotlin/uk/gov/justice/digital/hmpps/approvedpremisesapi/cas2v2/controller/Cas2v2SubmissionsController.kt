...
class Cas2v2SubmissionsController(
  ...
  fun submissionsGet(
    ...
    userService.ensureUserPersisted(Cas2ServiceOrigin.BAIL)

    val sortDirection = SortDirection.asc
    val sortBy = "submittedAt"

    val (applications, metadata) = cas2ApplicationService.getAllSubmittedCas2v2ApplicationsForAssessor(PageCriteria(sortBy, sortDirection, page))
  ...
  fun submissionsApplicationIdGet(
    ...
    userService.getUserForRequest(Cas2ServiceOrigin.BAIL)

    val applicationResult = cas2ApplicationService.getSubmittedCas2v2ApplicationForAssessor(applicationId)
  ...
  fun submissionsPost(
    ...
    val submitResult = cas2ApplicationService.submitCas2v2Application(submitCas2v2Application, user)
  ...
  private fun getPersonDetailAndTransform(
    ...
    val personInfo = when (val cas2v2OffenderSearchResult = cas2OffenderService.getPersonByNomisIdOrCrn(application.crn)) {
      ...
      is Cas2OffenderSearchResult.Unknown -> throw cas2v2OffenderSearchResult.throwable ?: BadRequestProblem(errorDetail = "Could not retrieve person info for Prison Number: ${application.crn}")
      is Cas2OffenderSearchResult.Success.Full -> cas2v2OffenderSearchResult.person
    }
