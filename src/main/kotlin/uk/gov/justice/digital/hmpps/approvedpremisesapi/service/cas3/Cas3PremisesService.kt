...
class Cas3PremisesService(
  ...
  fun updateBedspace(
    ...
    var room = roomRepository.findByIdOrNull(bedspace.room.id)
      ?: return CasResult.NotFound("Room", bedspace.room.id.toString())

    if (bedspace.room.premises != premises) {
      return CasResult.NotFound("Premises", premises.id.toString())
    }
    ...
    return success(room.beds.first())
  }
