  ...
  fun handleMessageNotReadableException(
    ...
    when (exception.cause) {
      is MismatchedInputException -> {
        val mismatchInputException = exception.cause as MismatchedInputException
        ...
        if (expectedArrayButGotObject(jsonTree, mismatchInputException)) {
        ...
        if (expectedObjectButGotArray(jsonTree, mismatchInputException)) {
          val problemDetail = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, "Expected an object but got an array")
          problemDetail.title = "Bad Request"
          return ResponseEntity.status(HttpStatus.BAD_REQUEST).contentType(APPLICATION_PROBLEM_JSON).body(problemDetail)
        }

        val badRequestProblem = if (rootIsArray(mismatchInputException)) {
          val arrayItemsType = (mismatchInputException.path[1].from() as Class<*>).kotlin
          ...
          val objectType = (mismatchInputException.path[0].from() as Class<*>).kotlin
