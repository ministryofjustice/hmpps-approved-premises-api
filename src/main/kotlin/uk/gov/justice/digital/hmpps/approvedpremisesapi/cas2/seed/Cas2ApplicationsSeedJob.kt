...
class Cas2ApplicationsSeedJob(
  ...
  override fun deserializeRow(columns: Map<String, String>) = Cas2ApplicationSeedCsvRow(
    ...
    cohort = SeedColumns(columns).getStringOrNull("cohort")?.let { Cas2Cohort.valueOf(it) },
  )
  ...
  override fun processRow(row: Cas2ApplicationSeedCsvRow) {
    ...
    val applicant = cas2UserRepository.findByUsernameAndServiceOrigin(row.createdBy, Cas2ServiceOrigin.BAIL) ?: throw RuntimeException("Could not find applicant with cas2v2user ${row.createdBy}")
  ...
  private fun findStatusAtPosition(idx: Int): Cas2PersistedApplicationStatus = Cas2PersistedApplicationStatusFinder.active(serviceName = ServiceName.cas2)[idx]
