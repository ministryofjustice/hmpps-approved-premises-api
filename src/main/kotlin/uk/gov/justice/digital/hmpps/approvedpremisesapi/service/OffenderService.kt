...
class OffenderService(
  ...
  fun getCsraSummariesForOffender(
    crn: String,
    laoStrategy: LaoStrategy.CheckUserAccess,
  ): CasResult<List<CsraSummary>> {
    cas1OffenderRepository.findByCrn(crn)?.nomsNumber?.let { nomsNumber ->
      return prisonsApiClient.getCsraSummariesForOffender(nomsNumber)
        .toCasResult(entityType = "CsraSummary", id = crn)
    }

    return when (val personSummaryInfoResult = getPersonSummaryInfoResult(crn, laoStrategy)) {
      is PersonSummaryInfoResult.Success.Full -> {
        personSummaryInfoResult.summary.nomsId?.let { nomsNumber ->
