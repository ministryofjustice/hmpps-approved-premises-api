...
interface PremisesApi {
  ...
  fun premisesPost(@Parameter(description = "", required = true) @RequestBody body: NewPremises, @Parameter(description = "If given, persist the service name against this property", `in` = ParameterIn.HEADER, schema = Schema(allowableValues = ["approved-premises", "cas2", "temporary-accommodation"])) @RequestHeader(value = "X-Service-Name", required = false) xServiceName: ServiceName?): ResponseEntity<Premises> {
  ...
  @Operation(
    ...
    operationId = "premisesPremisesIdCalendarGet",
    description = """""",
    responses = [
      ApiResponse(responseCode = "200", description = "successful operation", content = [Content(array = ArraySchema(schema = Schema(implementation = BedOccupancyRange::class)))]),
      ApiResponse(responseCode = "401", description = "not authenticated", content = [Content(schema = Schema(implementation = Problem::class))]),
      ApiResponse(responseCode = "403", description = "unauthorised", content = [Content(schema = Schema(implementation = Problem::class))]),
      ApiResponse(responseCode = "404", description = "invalid premises ID", content = [Content(schema = Schema(implementation = Problem::class))]),
      ApiResponse(responseCode = "500", description = "unexpected error", content = [Content(schema = Schema(implementation = Problem::class))]),
  ...
  @RequestMapping(
    method = [RequestMethod.GET],
    value = ["/premises/{premisesId}/calendar"],
    produces = ["application/json"],
  )
  fun premisesPremisesIdCalendarGet(@Parameter(description = "ID of the premises to get bed information for", required = true) @PathVariable("premisesId") premisesId: java.util.UUID, @Parameter(description = "Start of date range to return occupancy for", required = true) @RequestParam(value = "startDate", required = true) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) startDate: java.time.LocalDate, @Parameter(description = "End of date range to return occupancy for", required = true) @RequestParam(value = "endDate", required = true) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) endDate: java.time.LocalDate): ResponseEntity<List<BedOccupancyRange>> {
    return getDelegate().premisesPremisesIdCalendarGet(premisesId, startDate, endDate)
  }
