...
The Subject Access Request (SAR) API response often contains `null` values for optional fields that were not populated in the source system.

- **JSON Representation**: Missing data is represented as `null` in the JSON API response (e.g., `"reallocated_at": null`).
- **Template Rendering**: The Mustache template uses helpers like `{{ optionalValue fieldName }}` to handle these `null` values gracefully. When a value is `null`, it is typically rendered as "Not Provided" or left blank in the HTML report to ensure a clean and professional layout.